...
const CREATURE_LEFT := &"creature_left"       # {creature, reason, ...}
const CREATURE_CHANGED := &"creature_changed"        # {creature, change}
...
const CAR_SEARCHED := &"car_searched"                # {creature, tries, muttering}
const CAR_HOTWIRE_FAILED := &"car_hotwire_failed"    # {creature, vehicle, fumble}
...
const NEWS_PUBLISHED := &"news_published"            # {story, page, guardian, guardian_page, ...}
...
const JURY_SEATED := &"jury_seated"                  # {jury: count, manner}
