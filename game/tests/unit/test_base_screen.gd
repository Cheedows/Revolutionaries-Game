...
func test_the_courts_list_who_is_held() -> void:
	var session := _a_game(13579)
	var state := session.state
	var jailed := state.add_creature(Creature.new())
	jailed.name = "Ida Kell"
	jailed.alignment = &"liberal"
	jailed.join_days = 1
	jailed.sentence = 90
	jailed.confessions = 2
	jailed.crimes_suspected[Ids.LAW_FLAGS.find(&"arson")] = 1

	var cell := Location.new()
	cell.id = 90001
	cell.type = &"government_prison"
	state.locations[cell.id] = cell
	jailed.location = cell.id
