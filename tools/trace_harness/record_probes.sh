...
for probe in blank creatures training checks equipment politics activities damage congress elections court names opinion wincheck world spawn sitemaps sites context combat chase fight encounters stealth recruit activities_day activation recovery; do
	tmp="$(mktemp)"
	home="$(mktemp -d)"
	screen="$(mktemp)"
