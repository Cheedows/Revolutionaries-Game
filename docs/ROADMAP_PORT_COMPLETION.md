...
- [x] Replace legacy cutscenes/title ASCII with modern presentation hooks
    rather than porting broken 64-bit `.cmv/.cpc` playback. There are only
    eight of those files and every one is now a name a presentation can answer
    however it likes rather than a reel to play:
  - the five `.cmv` films are the television segments, named in
    `NewsBroadcast.FILMS` with their music in `BroadcastText.MUSIC` and the
    three lines each printed under them in `BroadcastText.LINES`;
  - `largecap.cpc` is the block-letter headline font, replaced by
    `HeadlineText`, which gives a headline as its lines and lets the interface
    set them;
  - `newstops.cpc` is the five mastheads, replaced by the masthead roll in
    `NewsReading._page` — the choice is the simulation's, the artwork is not;
  - `newspic.cpc` is the photographs half the major events run instead of a
    story, named in `MajorEventPageText.PICTURES` with their captions beside
    them;
  - and the title screen's own ASCII is replaced by the plain title, while the
    writing that mattered — the thirty-two quotations about disobeying the law,
    which are the only words in the game that are not about the game — is kept
    verbatim in `TitleQuotes`, generated by `tools/extract_quotes.py`, and one
    is chosen the way the original chooses.
