...
- Files stay small. Target ≤300 lines. Generated files are exempt — their
  length is the length of the thing they were lifted from — and are marked
  with a "GENERATED by" line near the top, which the checker looks for. `sitemode.cpp` at 2,784 lines becomes
  `site/infiltrate.gd` + `stealth.gd` + `alarm.gd` + `talk.gd` + `specials.gd` + `escape.gd`.
